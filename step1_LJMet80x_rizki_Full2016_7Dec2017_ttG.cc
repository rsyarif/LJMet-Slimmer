...
//    TLorentzVector lepton_lv;
  ...
  // ----------------------------------------------------------------------------
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if( (elPt_singleLepCalc->size()+muPt_singleLepCalc->size()) <3) continue; //skip if there are less than 3 leptons to begin with.
...
// 		if(!isTightMu && isLooseMu){ //ATTENTION: WHICH ONE iS CORRECT???
      ...
      if(AllLeptonCount<3) continue; //skip if there are less than 3 leptons.
